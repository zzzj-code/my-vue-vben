    ...
    <div class="app">
      <div class="app-top">
        <div class="top-inp">
          <div>
            <span>盘库单号</span>
            <input type="text" placeholder="请输入盘库单号" v-model="searchForm.orderNo" />
          </div>
          <div>
            <span>业务单号</span>
      ...
      <div class="app-main">
        <div class="main-top">
          <div>盘库单列表</div>
          <div>
            <button @click="handleAdd">+新增盘库单</button>
            <button>导出</button>
            <button>🔍</button>
          </div>
          <div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th><div class="th-inner">入库状态</div></th>
                <th><div class="th-inner">入库类型</div></th>
  ...
  methods: {
    ...
    async loadCheckList() {
      try {
        ...
        const data = await getGoodsCommonOperationOrderPage({
          ...
          // type: "盘库", // 如需按业务类型过滤（出库/移库/盘库），可加该参数，值以后端枚举为准
        });
        const list = (data && data.list) || [];
        // 字段映射：后端字段 → 页面模板字段（模板列：单号/状态/类型/仓库/数量金额/供应商/操作信息/备注）
        this.tabValue = list.map((item) => ({
          id: item.id,                                        // 主键，删除时使用
          orderNo: item.no || item.orderNo || "",             // 盘库单号
          businessNo: item.businessNo || item.salesOrderNo || "", // 业务单号
          status: this.formatStatus(item.status),             // 入库状态（数字枚举 → 中文）
          type: this.formatType(item.type),                   // 入库类型
    ...
    async handleDelete(row) {
      ...
      try {
        ...
        console.error("删除盘库单失败", err);
      }
    },
    // ========== 打印 ==========
    handlePrint(row) {
      alert(`打印盘库单：${row.orderNo}`);
    },
