    ...
    <div class="app">
      <div class="app-top">
        <div class="top-inp">
          <div>
            <span>出库单号</span>
            <input type="text" placeholder="请输入出库单号" v-model="searchForm.orderNo" />
          </div>
          <div>
            <span>业务单号</span>
      ...
      <div class="app-main">
        <div class="main-top">
          <div>出库单列表</div>
          <div>
            <button @click="handleAdd">+新增出库单</button>
            <button>导出</button>
            <button>🔍</button>
          </div>
          <div>
...
// 说明：出库单在本项目中对应"通用操作单"（goods-common-operation-order）。
//       WMS 只有 4 个 API 文件，出库/移库/盘库均走这个通用操作单接口，用 type 字段区分业务类型。
// getShipmentOrderPage : 通用操作单分页查询（GET /wms/goods-common-operation-order/page）
// deleteShipmentOrder  : 删除通用操作单（DELETE /wms/goods-common-operation-order/delete?id=xxx）
import { getShipmentOrderPage, deleteShipmentOrder } from '#/api/wms/shipment-order';
...
export default {
  ...
  methods: {
    ...
    async loadShipmentList() {
      try {
        ...
        const data = await getShipmentOrderPage({
          ...
          no: this.searchForm.orderNo,             // 出库单号
          businessNo: this.searchForm.businessNo,  // 业务单号
          // type: "出库", // 如需按业务类型过滤（出库/移库/盘库），可加该参数，值以后端枚举为准
        });
        const list = (data && data.list) || [];
        // 字段映射：后端字段 → 页面模板字段
        this.tabValue = list.map((item) => ({
          id: item.id,                                        // 主键，删除时使用
          orderNo: item.no || item.orderNo || "",             // 出库单号
          businessNo: item.businessNo || item.salesOrderNo || "", // 业务单号（销售订单号等）
    ...
    // ========== 类型格式化（兼容数字枚举 / 中文字符串）==========
    formatType(type) {
      const map = { 1: '销售出库', 2: '退货出库', 3: '调拨出库' };
