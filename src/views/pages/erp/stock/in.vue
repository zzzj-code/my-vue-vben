<template>
  <div class="page-wrapper">
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-top">
          <div class="top-1">其它入库单列表</div>
          <div class="top-2">
            <button @click="handleAdd">+新增其他入库单</button>
            <button>导出</button>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in tabValue" :key="item.id">
                <td class="col-check"><input type="checkbox" name="" id="" disabled /></td>
...
<script>
// ========== 导入其他入库相关API ==========
import { getStockInPage, deleteStockIn } from '#/api/erp/stock/in';
...
export default {
  ...
  methods: {
    ...
    async loadStockInList() {
      try {
        const data = await getStockInPage({
        ...
        this.tabValue = data.list.map((item) => ({
          ...
          status: item.status === 20 ? "已入库" : "待入库", // 状态
        }));
        this.pagination.total = data.total;
      } catch (err) {
        console.error("获取其他入库列表失败", err);
      }
    },
    // ========== 时间戳格式化 ==========
