    ...
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-title">
          ...
          <div>
            <button @click="loadTransferList">⟳</button>
            <button>⛶</button>
            <button>⊞</button>
          </div>
        </div>
...
<script>
// ========== 导入员工调动申请相关API ==========
// getEmployeeTransferBillPage: 分页查询调动申请列表
// getEmployeeTransferBill: 获取单条调动申请详情
// saveEmployeeTransferBill: 保存调动申请（草稿）
// submitEmployeeTransferBill: 提交调动申请
// updateEmployeeTransferBill: 更新调动申请
// deleteEmployeeTransferBill: 删除调动申请
import { getEmployeeTransferBillPage, deleteEmployeeTransferBill } from '#/api/hrm/employee-transfer';
...
export default {
  ...
  methods: {
    ...
    async loadTransferList() {
      try {
        const data = await getEmployeeTransferBillPage({
    ...
    // ========== 查看详情 ==========
    handleDetail(row) {
      alert(`调动申请详情：${row.billCode}`);
    },

    // ========== 删除调动申请 ==========
    async handleDelete(row) {
      if (!confirm(`确定要删除调动申请「${row.billCode}」吗？`)) return;
      try {
        await deleteEmployeeTransferBill(row.id);
        alert("删除成功");
        this.loadTransferList();
      } catch (err) {
        console.error("删除调动申请失败", err);
      }
    },
  },
};
