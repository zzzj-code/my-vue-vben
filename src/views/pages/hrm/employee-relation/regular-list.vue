    ...
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-title">
          ...
          <div>
            <button @click="loadRegularList">⟳</button>
            <button>⛶</button>
            <button>⊞</button>
          </div>
        </div>
...
<script>
// ========== 导入员工转正申请相关API ==========
// getEmployeeRegularBillPage: 分页查询转正申请列表
// getEmployeeRegularBill: 获取单条转正申请详情
// saveEmployeeRegularBill: 保存转正申请（草稿）
// submitEmployeeRegularBill: 提交转正申请
// updateEmployeeRegularBill: 更新转正申请
// deleteEmployeeRegularBill: 删除转正申请
import { getEmployeeRegularBillPage, deleteEmployeeRegularBill } from '#/api/hrm/employee-regular';
...
export default {
  ...
  // ========== 页面挂载后自动加载列表 ==========
  mounted() {
    this.loadRegularList();
  },

  methods: {
    // ========== 接口对接方法：获取转正申请列表 ==========
    async loadRegularList() {
      try {
        // 调用分页查询接口，传入页码、每页条数和搜索条件
        const data = await getEmployeeRegularBillPage({
    ...
    // ========== 查看详情 ==========
    handleDetail(row) {
      alert(`转正申请详情：${row.billCode}`);
    },

    // ========== 删除转正申请 ==========
    async handleDelete(row) {
      if (!confirm(`确定要删除转正申请「${row.billCode}」吗？`)) return;
      try {
        await deleteEmployeeRegularBill(row.id);
        alert("删除成功");
        this.loadRegularList();
      } catch (err) {
        console.error("删除转正申请失败", err);
      }
    },
  },
};
