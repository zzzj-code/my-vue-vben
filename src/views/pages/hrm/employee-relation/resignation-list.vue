    ...
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-title">
          ...
          <div>
            <button @click="loadResignationList">⟳</button>
            <button>⛶</button>
            <button>⊞</button>
          </div>
        </div>
...
<script>
// ========== 导入员工离职申请相关API ==========
// getEmployeeResignationBillPage: 分页查询离职申请列表
// getEmployeeResignationBill: 获取单条离职申请详情
// saveEmployeeResignationBill: 保存离职申请（草稿）
// submitEmployeeResignationBill: 提交离职申请
// updateEmployeeResignationBill: 更新离职申请
// deleteEmployeeResignationBill: 删除离职申请
import { getEmployeeResignationBillPage, deleteEmployeeResignationBill } from '#/api/hrm/employee-resignation';
...
export default {
  ...
  methods: {
    ...
    async loadResignationList() {
      try {
        const data = await getEmployeeResignationBillPage({
    ...
    // ========== 重置按钮 ==========
    handleReset() {
      this.searchForm = { billCode: "", processStatus: "", employeeNo: "" };
      this.pagination.pageNo = 1;
      this.loadResignationList();
    },
    ...
    // ========== 查看详情 ==========
    handleDetail(row) {
      alert(`离职申请详情：${row.billCode}`);
    },

    // ========== 删除离职申请 ==========
    async handleDelete(row) {
      if (!confirm(`确定要删除离职申请「${row.billCode}」吗？`)) return;
      try {
        await deleteEmployeeResignationBill(row.id);
        alert("删除成功");
        this.loadResignationList();
      } catch (err) {
        console.error("删除离职申请失败", err);
      }
    },
  },
};
