    ...
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-top">
          <div>采购入库单列表</div>
          <div>
            <button @click="handleAdd">+新增采购入库单</button>
            <button>导出</button>
            <button>🔍</button>
          </div>
          <div>
...
<script>
// ========== 导入物料入库相关API ==========
import { getItemReceiptPage, deleteItemReceipt } from '#/api/mes/wm/item-receipt';
...
export default {
  ...
  methods: {
    ...
    async loadList() {
      try {
        const data = await getItemReceiptPage({
