    ...
    <div class="app">
      <div class="app-top">
        <div class="top-inp">
          <div>
            <span>缺陷编码</span>
            <input type="text" placeholder="请输入缺陷编码" v-model="searchForm.field1" />
          </div>
          <div>
            <span>缺陷描述</span>
            <input type="text" placeholder="请输入缺陷描述" v-model="searchForm.field2" />
          </div>
          <div>
      ...
      <div class="app-main">
        <div class="main-top">
          <div>缺陷类型列表</div>
          <div>
            <button @click="handleAdd">+新增缺陷类型</button>
            <button>导出</button>
            <button>🔍</button>
          </div>
          <div>
...
<script>
// ========== 导入缺陷管理相关API ==========
import { getDefectPage, deleteDefect } from '#/api/mes/qc/defect';
...
export default {
  ...
  methods: {
    ...
    async loadList() {
      try {
        const data = await getDefectPage({
