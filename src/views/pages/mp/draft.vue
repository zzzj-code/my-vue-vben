    ...
    <div class="app">
      ...
      <div class="app-main">
        <div class="main-top">
          <div>草稿列表</div>
          <div>
            <button @click="handleAdd">+新增图文草稿</button>
            <button>🔍</button>
          </div>
          <div>
            <button>⟳</button>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th><div class="th-cell">修改时间</div></th>
                <th class="ol-col"><div class="th-cell">操作</div></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in tabValue" :key="item.id">
                <td><img :src="item.thumbUrl" style="width:60px;height:40px" v-if="item.thumbUrl" /></td>
                <td>{{ item.title }}</td>
...
<script>
// ========== 导入公众号草稿相关API ==========
import { getDraftPage, deleteDraft } from '#/api/mp/draft';
  ...
  methods: {
    ...
    async loadList() {
      try {
        ...
        const data = await getDraftPage(params);
    ...
    async handleDelete(item) {
      ...
      try {
        await deleteDraft(this.searchForm.accountId, item.id);
        alert('删除成功');
        this.loadList();
      } catch (err) {
