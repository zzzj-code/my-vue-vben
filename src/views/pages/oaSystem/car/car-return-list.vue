    ...
    <div class="app">
        ...
        <form @submit.prevent="handleSearch">
          <div class="top-tab">
            <div class="top-row">
              ...
              <div class="tab-a">
                <span>车辆</span>
                <input type="text" placeholder="请输入车辆" v-model="searchForm.carNo" />
              </div>
              <div class="tab-a">
                <span>用车申请表</span>
                <input type="text" placeholder="请选择用车申请表" v-model="searchForm.applyBill" />
              </div>
            </div>
            <div class="top-row">
              <div class="tab-a">
                <span>申请部门</span>
                <input type="text" placeholder="请输入申请部门" v-model="searchForm.deptName" />
              </div>
              <div class="tab-a">
                <span>创建时间</span>
                <input type="text" placeholder="请选择创建时间" />
              </div>
              <div class="tab-b">
      ...
      <div class="app-main">
        <div class="main-header">
          <div class="main-title">还车申请列表</div>
          <div class="main-buttons">
            ...
            <button class="icon-btn" @click="loadReturnList">⟳</button>
...
<script>
// ========== 导入还车登记相关API ==========
// getCarReturnBillPage: 分页查询还车登记列表
// getCarReturnBill: 获取单条还车登记详情
// saveCarReturnBill: 保存还车登记（草稿）
// submitCarReturnBill: 提交还车登记
// updateCarReturnBill: 更新还车登记
// deleteCarReturnBill: 删除还车登记
import { getCarReturnBillPage, deleteCarReturnBill } from '#/api/oa/car/carreturn';
  ...
  methods: {
    ...
    async loadReturnList() {
      try {
        ...
        // cause -> summary（还车事由，作为摘要显示）
        // companyName -> company（所属公司）
        // deptName -> department（所属部门）
        // processStatus -> processStatus（流程状态：1=审批中，2=已完成，3=已取消，4=已驳回，-1=草稿）
        // createTime -> startTime（创建时间，作为发起时间）
        // returnTime -> endTime（实际还车时间，作为结束时间）
        this.rows = data.list.map((item) => ({
          ...
          endTime: this.formatTimestamp(item.returnTime),
        }));
        ...
        // 接口调用失败时打印错误信息
        console.error("获取还车登记列表失败", err);
      }
    },

    // ========== 工具方法：流程状态编码转中文文本 ==========
    getStatusText(status) {
      const map = { "-1": "草稿", 1: "审批中", 2: "已完成", 3: "已取消", 4: "已驳回" };
      return map[status] || "未知";
    },

    // ========== 工具方法：流程状态对应样式类名 ==========
    // 不同状态显示不同颜色：草稿=灰色，审批中=蓝色，已完成=绿色，已取消=灰色，已驳回=红色
    getStatusClass(status) {
      const map = { "-1": "status-gray", 1: "status-blue", 2: "status-green", 3: "status-gray", 4: "status-red" };
    ...
    handleReset() {
      // 清空搜索条件
      this.searchForm = { billCode: "", processStatus: "", carNo: "", applyBill: "", deptName: "" };
      // 重置到第一页
      this.pagination.pageNo = 1;
      this.loadReturnList();
    },
    ...
    handleDetail(row) {
      // TODO: 打开还车登记详情弹窗
      alert(`还车登记详情：${row.number}`);
    },

    // ========== 删除还车登记 ==========
    async handleDelete(row) {
      // 弹出确认框，防止误删
      if (!confirm(`确定要删除还车登记「${row.number}」吗？`)) return;
      try {
        // 调用删除接口
        await deleteCarReturnBill(row.id);
        alert("删除成功");
        // 删除成功后重新加载列表
        this.loadReturnList();
      } catch (err) {
        console.error("删除还车登记失败", err);
      }
    },
  },
};
