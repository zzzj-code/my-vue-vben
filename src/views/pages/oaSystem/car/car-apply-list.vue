    ...
    <div class="app">
        ...
        <form @submit.prevent="handleSearch">
          <div class="top-tab">
            <div class="top-row">
              ...
              <div class="tab-a">
                <span>申请部门</span>
                <input type="text" placeholder="请选择申请部门" v-model="searchForm.deptName" />
              </div>
            </div>
            <div class="top-row">
              <div class="tab-a">
              ...
              <div class="tab-a">
                <span>还车状态</span>
                <input type="text" placeholder="请选择还车状态" v-model="searchForm.returnStatus" />
              </div>
              <div class="tab-b">
      ...
      <div class="app-main">
        <div class="main-header">
          <div class="main-title">用车申请列表</div>
...
<script>
// ========== 导入用车申请相关API ==========
// getCarApplyBillPage: 分页查询用车申请列表
// getCarApplyBill: 获取单条申请详情
// saveCarApplyBill: 保存申请（草稿）
// submitCarApplyBill: 提交申请
// updateCarApplyBill: 更新申请
// deleteCarApplyBill: 删除申请
import { getCarApplyBillPage, deleteCarApplyBill } from '#/api/oa/car/carapply';
  ...
  methods: {
    ...
    async loadApplyList() {
      try {
        ...
        const data = await getCarApplyBillPage({
        ...
        // returnTime -> endTime（预计返回时间，作为结束时间）
    ...
    handleReset() {
      // 清空搜索条件
      this.searchForm = { billCode: "", processStatus: "", carNo: "", deptName: "", returnStatus: "" };
      // 重置到第一页
      this.pagination.pageNo = 1;
      this.loadApplyList();
    },
    ...
    handleDetail(row) {
      // TODO: 打开申请详情弹窗
      alert(`用车申请详情：${row.number}`);
    },

    // ========== 删除申请 ==========
    async handleDelete(row) {
      // 弹出确认框，防止误删
      if (!confirm(`确定要删除用车申请「${row.number}」吗？`)) return;
      try {
        // 调用删除接口
        await deleteCarApplyBill(row.id);
        alert("删除成功");
        // 删除成功后重新加载列表
        this.loadApplyList();
      } catch (err) {
        console.error("删除用车申请失败", err);
      }
    },
  },
};
