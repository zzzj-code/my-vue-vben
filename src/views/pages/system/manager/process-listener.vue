        ...
        <form @submit.prevent="handleSearch">
          <div class="top-tab">
            <div class="top-row">
              ...
              <div class="tab-a">
                <span>类型</span>
                <input type="text" placeholder="请输入类型" v-model="searchForm.type" />
              </div>
              <div class="tab-b">
      ...
      <div class="app-main">
        <div class="main-header">
          ...
          <div class="main-buttons">
            <button class="btn btn-primary" @click="handleAdd">+ 新增流程监听器</button>
            <button class="icon-btn">🔍</button>
            <button class="icon-btn" @click="loadListenerList">⟳</button>
            <button class="icon-btn">⛶</button>
            <button class="icon-btn">☷</button>
          </div>
        </div>
...
<script>
// 导入流程监听器相关API
import { getProcessListenerPage, deleteProcessListener } from '#/api/bpm/processListener';
...
export default {
  ...
  mounted() {
    this.loadListenerList();
  },
  methods: {
    // 获取流程监听器列表
    async loadListenerList() {
      try {
        ...
        this.rows = data.list.map((item) => ({
          ...
          type: item.type || "",
          event: item.event || "",
          className: item.className || item.expression || "",
    ...
    // 编辑
    handleEdit(row) {
      alert(`编辑流程监听器：${row.name}`);
    },
    // 删除
    async handleDelete(row) {
      if (!confirm(`确定要删除流程监听器「${row.name}」吗？`)) return;
      try {
        await deleteProcessListener(row.id);
        alert("删除成功");
        this.loadListenerList();
      } catch (err) {
        console.error("删除流程监听器失败", err);
      }
    },
  },
};
