        ...
        <form @submit.prevent="handleSearch">
          <div class="top-tab">
            <div class="top-row">
              <div class="tab-a">
                <span>流程名称</span>
                <input type="text" placeholder="请输入流程名称" v-model="searchForm.processInstanceName" />
              </div>
              <div class="tab-a">
                <span>抄送时间</span>
                <input type="text" placeholder="请选择抄送时间" />
              </div>
              <div class="tab-b">
...
export default {
  ...
  methods: {
    ...
    async loadCopyList() {
      try {
        ...
        this.rows = data.list.map((item) => ({
          ...
          startTime: this.formatTimestamp(item.processInstanceStartTime),
          endTime: this.formatTimestamp(item.createTime),
        }));
        this.pagination.total = data.total;
      } catch (err) {
        console.error("获取抄送我的流程失败", err);
      }
    },
    // 从摘要数组中提取值
    ...
    // 重置
    handleReset() {
      this.searchForm = { processInstanceName: "" };
      this.pagination.pageNo = 1;
      this.loadCopyList();
    },
    // 分页
