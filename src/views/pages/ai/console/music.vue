...
<script>
import { getMusicPage, deleteMusic } from '#/api/ai/music';
export default {
  ...
  methods: {
    ...
    async handleDelete(item) { if (!confirm('确定要删除吗？')) return; try { await deleteMusic(item.id); alert('删除成功'); this.loadList(); } catch (err) { console.error('删除失败', err); } },
  },
};
</script>
<style scoped>
