    ...
    <div class="app">
      <div class="app-top">
        <div class="top-box">
          <div>
            <span>商机名称</span>
            <input type="text" placeholder="请输入商机名称" v-model="searchForm.name" />
          </div>
          <div>
          </div>
          <div>
      ...
      <div class="app-main">
        <div class="main-top">
          ...
          <div class="top-2">
            <button @click="handleAdd">+新增商机</button>
            <button>导出</button>
            <button @click="handleSearch">🔍</button>
          </div>
          <div class="top-3">
            <button @click="loadBusinessList">⟳</button>
            <button>⛶</button>
            <button>⊞</button>
          </div>
        </div>
...
<script>
// ========== 导入商机管理相关API ==========
// getBusinessPage: 分页查询商机列表
// getBusiness: 获取单条商机详情
// createBusiness: 创建商机
// updateBusiness: 更新商机
// deleteBusiness: 删除商机
import { getBusinessPage, deleteBusiness } from '#/api/crm/business';
...
export default {
  ...
  methods: {
    ...
    async handleDelete(row) {
      if (!confirm(`确定要删除商机「${row.name}」吗？`)) return;
      try {
