    ...
    <div class="app">
      <div class="app-top">
        <div class="top-box">
          <div>
            <span>回款编号</span>
            <input type="text" placeholder="请输入回款编号" v-model="searchForm.no" />
          </div>
          <div>
            <span>客户</span>
      ...
      <div class="app-main">
        <div class="main-top">
          ...
          <div class="top-2">
            <button @click="handleAdd">+新增回款</button>
            <button>导出</button>
            <button @click="handleSearch">🔍</button>
          </div>
          <div class="top-3">
            <button @click="loadReceivableList">⟳</button>
            <button>⛶</button>
            <button>⊞</button>
          </div>
        </div>
...
<script>
// ========== 导入回款管理相关API ==========
import { getReceivablePage, deleteReceivable } from '#/api/crm/receivable';
...
export default {
  ...
  methods: {
    async loadReceivableList() {
      try {
        const data = await getReceivablePage({
    ...
    handleSearch() { this.pagination.pageNo = 1; this.loadReceivableList(); },
    handleReset() { this.searchForm = { no: "", customerName: "" }; this.pagination.pageNo = 1; this.loadReceivableList(); },
    handlePageChange(page) { this.pagination.pageNo = page; this.loadReceivableList(); },
    handleAdd() { alert("新增回款功能待实现"); },
    handleEdit(row) { alert(`编辑回款：${row.name}`); },
    async handleDelete(row) {
      if (!confirm(`确定要删除回款「${row.name}」吗？`)) return;
      try { await deleteReceivable(row.id); alert("删除成功"); this.loadReceivableList(); }
      catch (err) { console.error("删除回款失败", err); }
    },
  },
};
</script>
